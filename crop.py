...
def cropper(h_lines_final, v_lines_final, img, write=False, path = None):



    '''
    create pairs of adjcent lines and get the intersection points. crop it from the original image. 
    '''    
    ...
    for i in range(len(h_lines_final)-1):
        ...
        for j in range(len(v_lines_final)-1):
            ...
            # contengency when lines are intersecting out of image plane
            ...
            if write:
                try: 
                    name = os.path.join(path, str(i)+str(i+1) + '_' + str(j)+str(j+1) + '.jpg')
                    # print(name)
                    cv2.imwrite(name, c)
