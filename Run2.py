...
try:
    ...
    from lines2 import getLines, getMargins
...
if __name__ == '__main__':
    inp_dir = "/content/drive/MyDrive/Trial"
    
    # Process the images to create halves
    halfcrop(inp_dir)

    halfcrop_dir = os.path.join(inp_dir, 'halfcrop')
    for subdir in os.listdir(halfcrop_dir):
        subdir_path = os.path.join(halfcrop_dir, subdir)
        
        if os.path.isdir(subdir_path):
            image_files = [os.path.join(subdir_path, f) for f in os.listdir(subdir_path) if f.lower().endswith(('.jpg', '.png'))]
